...
const bullets = [
  ...
  {
    prefix: null,
    text: "Ghost subscriptions: customers marked \u201Cactive\u201D who stopped using your product months ago. They inflate your MRR. And when they finally notice the charge, they don\u2019t just cancel. They chargeback.",
  },
  {
    prefix: "Here\u2019s one that\u2019s easy to miss:",
  ...
  {
    prefix: "When was the last time you checked",
    text: "which credit cards are expiring next month? Stripe won\u2019t proactively alert you. You only find out when the payment fails. And by then, some of those customers are already gone.",
  },
  {
    prefix: null,
    text: "$129 billion is lost every year to failed payments alone (Recurly, 2024). That\u2019s just one of 10 leak types. The other 9? Most founders have never even looked.",
  },
];
  ...
  return (
    <section className="relative py-20 md:py-28">
      ...
      <div className="relative mx-auto max-w-4xl px-6 pt-16">
        <div className="mb-3 text-[13px] text-white/50 italic">
          What you don&apos;t know
        </div>
        <h2 className="mb-12 font-display text-3xl font-bold tracking-tight text-white md:text-4xl lg:text-5xl">
          ...
          {bullets.map((bullet, i) => (
            ...
            >
              <p className="text-sm leading-[1.8] text-white/45">
                {bullet.prefix ? (
                  <>
                    <span className="font-semibold text-white/70">{bullet.prefix}</span>{" "}
                    {bullet.prefix.endsWith(":") ? "" : "that "}
                  </>
                ) : null}
                {bullet.text}
              </p>
